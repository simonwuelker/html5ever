    ...
    fn step(&self, mode: InsertionMode, token: Token) -> ProcessResult<Handle> {
        ...
        match mode {
            ...
            InsertionMode::Initial => match_token!(token {
                Token::CharacterTokens(SplitStatus::NotSplit, text) => ProcessResult::SplitWhitespace(text),
                Token::CharacterTokens(SplitStatus::Whitespace, _) => ProcessResult::Done,
                Token::CommentToken(text) => self.append_comment_to_doc(text),
                token => {
            ...
            InsertionMode::BeforeHead => match_token!(token {
                Token::CharacterTokens(SplitStatus::NotSplit, text) => ProcessResult::SplitWhitespace(text),
                Token::CharacterTokens(SplitStatus::Whitespace, _) => ProcessResult::Done,
                Token::CommentToken(text) => self.append_comment(text),
            ...
            InsertionMode::InHead => match_token!(token {
                Token::CharacterTokens(SplitStatus::NotSplit, text) => ProcessResult::SplitWhitespace(text),
                Token::CharacterTokens(SplitStatus::Whitespace, text) => self.append_text(text),
                Token::CommentToken(text) => self.append_comment(text),
            ...
            InsertionMode::InHeadNoscript => match_token!(token {
                ...
                Token::CharacterTokens(SplitStatus::NotSplit, text) => ProcessResult::SplitWhitespace(text),
                Token::CharacterTokens(SplitStatus::Whitespace, _) => self.step(InsertionMode::InHead, token),

                Token::CommentToken(_) => self.step(InsertionMode::InHead, token),
            ...
            InsertionMode::InBody => match_token!(token {
                Token::NullCharacterToken => self.unexpected(&token),

                Token::CharacterTokens(_, text) => {
                ...
                tag @ </br> => {
                    self.unexpected(&tag);
                    self.step(InsertionMode::InBody, Token::TagToken(Tag {
                        kind: StartTag,
                        attrs: vec!(),
                        ..tag
                    }))
            ...
            InsertionMode::Text => match_token!(token {
                Token::CharacterTokens(_, text) => self.append_text(text),
            ...
            InsertionMode::InTable => match_token!(token {
                ...
                Token::NullCharacterToken => self.process_chars_in_table(token),

                Token::CharacterTokens(..) => self.process_chars_in_table(token),
                ...
                tag @ <input> => {
                    ...
                    if self.is_type_hidden(&tag) {
                        ...
                        self.foster_parent_in_body(Token::TagToken(tag))
                    }
                }
                ...
                Token::EOFToken => self.step(InsertionMode::InBody, token),
            ...
            InsertionMode::InTableText => match_token!(token {
                ...
                Token::CharacterTokens(split, text) => {
                ...
                token => {
                    ...
                    if contains_nonspace {
                        ...
                        for (split, text) in pending.into_iter() {
                            match self.foster_parent_in_body(Token::CharacterTokens(split, text)) {
                                ProcessResult::Done => (),
                  ...
                  <th> <thead> <tr> </table> </caption> => {
                    if self.in_scope_named(table_scope, local_name!("caption")) {
                        ...
                        match tag {
                            ...
                            _ => ProcessResult::Reprocess(InsertionMode::InTable, Token::TagToken(tag))
                        }
                    } else {
                        self.unexpected(&tag);
                        ProcessResult::Done
                ...
                tag @ </tbody> </tfoot> </thead> => {
                    if self.in_scope_named(table_scope, tag.name.clone()) {
                        if self.in_scope_named(table_scope, local_name!("tr")) {
                            ...
                            let node = self.pop();
                            self.assert_named(&node, local_name!("tr"));
                            ProcessResult::Reprocess(InsertionMode::InTableBody, Token::TagToken(tag))
                        } else {
                            ProcessResult::Done
                        }
                    } else {
            ...
            InsertionMode::InCell => match_token!(token {
                ...
                tag @ </table> </tbody> </tfoot> </thead> </tr> => {
                    if self.in_scope_named(table_scope, tag.name.clone()) {
                        self.close_the_cell();
                        ProcessResult::Reprocess(InsertionMode::InRow, Token::TagToken(tag))
                    } else {
                        self.unexpected(&tag)
                    }
                }
            ...
            InsertionMode::InSelectInTable => match_token!(token {
                ...
                tag @ </caption> </table> </tbody> </tfoot> </thead> </tr> </td> </th> => {
                    ...
                    if self.in_scope_named(table_scope, tag.name.clone()) {
                        self.pop_until_named(local_name!("select"));
                        ProcessResult::Reprocess(self.reset_insertion_mode(), Token::TagToken(tag))
                    } else {
                        ProcessResult::Done
                    }
                }
            ...
            InsertionMode::InTemplate => match_token!(token {
                Token::CharacterTokens(_, _) => self.step(InsertionMode::InBody, token),
                Token::CommentToken(_) => self.step(InsertionMode::InBody, token),
                ...
                tag @ <_> => {
                    ...
                    ProcessResult::Reprocess(InsertionMode::InBody, Token::TagToken(tag))
                }
            ...
            InsertionMode::AfterBody => match_token!(token {
                Token::CharacterTokens(SplitStatus::NotSplit, text) => ProcessResult::SplitWhitespace(text),
                Token::CharacterTokens(SplitStatus::Whitespace, _) => self.step(InsertionMode::InBody, token),
                Token::CommentToken(text) => self.append_comment_to_html(text),
            ...
            tag @ </_> => {
                ...
                loop {
                    ...
                    if !first && html {
                        ...
                        return self.step(mode, Token::TagToken(tag));
                    }
